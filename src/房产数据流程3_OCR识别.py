...
def extract_area(text):
    ...
    返回:
        str - 提取的面积字符串，格式如"100"（平方米）
    """
    area = ""
    
    area_patterns = [
        r"(\d+(?:\.\d+)?)\s*(?:[m㎡㎡㎡]|平方|平米)",  # 100m、100.5㎡等
        r"(\d+(?:\.\d+)?)\s*㎡",  # 100㎡
        r"(\d+(?:\.\d+)?)\s*ml",  # OCR识别错误的情况
        r"(\d+(?:\.\d+)?)\s*ML",  # OCR识别错误的情况
    ]
...
def extract_and_identify_person(text):
    ...
    """
    # 非人名黑名单
    non_person_keywords = {"东莞站", "贝壹", "贝联", "林泽武", "租赁", "相信自己", "再创佳绩", "今日共开", "贝壳开单", "喜报来", "开单套报", "开单喜报", "喜报合作", "合作共赢", "共创辉煌", "地产", "成交楼盘", "成交项目"}
...
def match_and_complement_secondhand_medium(all_results):
    """
    二手和中盘喜报关联匹配与信息互补
    ...
    1. 二手喜报缺少面积时，从中盘喜报补充
    2. 中盘喜报缺少金额时，从二手喜报补充
    3. 匹配条件：日期、小区、人员相同或匹配
    
    参数:
        all_results: list - 所有识别结果列表
    ...
    # 分离二手和中盘记录（包括标记为_skip_write的中盘记录，用于面积互补）
    secondhand_records = []
    medium_records = []
    
    for record in all_results:
        if record["类型"] == "二手":
            secondhand_records.append(record)
        elif record["类型"] == "中盘":
            medium_records.append(record)
    
    print(f"二手记录: {len(secondhand_records)} 条")
    print(f"中盘记录: {len(medium_records)} 条（含 {sum(1 for r in medium_records if r.get('_skip_write'))} 条待跳过记录）")
    ...
    print("\n调试信息：")
    print("二手记录:")
    for i, rec in enumerate(secondhand_records):
        print(f"  {i+1}. {rec['日期']} - {rec['小区']} - {rec['人员']} - 面积:'{rec['面积']}' - 金额:'{rec['金额']}'")
    print("中盘记录:")
    for i, rec in enumerate(medium_records):
        print(f"  {i+1}. {rec['日期']} - {rec['小区']} - {rec['人员']} - 面积:'{rec['面积']}' - 金额:'{rec['金额']}'")
    print()
    
    # 二手喜报缺少面积，尝试从中盘补充
    for sh_record in secondhand_records:
        if not sh_record["面积"]:
            found = False
            for m_record in medium_records:
                if is_match(sh_record, m_record):
                    if m_record["面积"]:
                        print(f"  [互补] 二手记录缺少面积，从中盘补充: {sh_record['小区']} - {sh_record['人员']} - 补充面积: {m_record['面积']}")
                        sh_record["面积"] = m_record["面积"]
                        if "备注" in sh_record and sh_record["备注"]:
                            sh_record["备注"] += f" [面积自中盘喜报]"
                        else:
                            sh_record["备注"] = "[面积自中盘喜报]"
                        match_count += 1
                        found = True
                        break
            if not found:
                print(f"  [未匹配] 二手记录未找到匹配的中盘记录: {sh_record['小区']} - {sh_record['人员']}")
    
    # 中盘喜报缺少金额，尝试从二手补充
    for m_record in medium_records:
        if m_record["金额"] == "0" or not m_record["金额"]:
            found = False
            for sh_record in secondhand_records:
                if is_match(m_record, sh_record):
                    if sh_record["金额"] and sh_record["金额"] != "0":
                        print(f"  [互补] 中盘记录缺少金额，从二手补充: {m_record['小区']} - {m_record['人员']} - 补充金额: {sh_record['金额']}")
                        m_record["金额"] = sh_record["金额"]
                        if "备注" in m_record and m_record["备注"]:
                            m_record["备注"] += f" [金额自二手喜报]"
                        else:
                            m_record["备注"] = "[金额自二手喜报]"
                        match_count += 1
                        found = True
                        break
            if not found:
                print(f"  [未匹配] 中盘记录未找到匹配的二手记录: {m_record['小区']} - {m_record['人员']}")
    
    print(f"完成互补: 共 {match_count} 条记录")
    print("=" * 60)
...
def is_match(record1, record2):
    ...
    参数:
        record1, record2: dict - 两条记录
    
    返回:
        bool - 是否匹配
    """
    # 日期匹配
    if record1["日期"] != record2["日期"]:
        return False
    
    # 小区匹配
    if record1["小区"] and record2["小区"]:
        # 小区相同或包含
        if record1["小区"] not in record2["小区"] and record2["小区"] not in record1["小区"]:
            return False
    elif record1["小区"] or record2["小区"]:
        # 一方有小区另一方没有，不算匹配
        return False
    
    # 对于二手和中盘喜报的匹配，不需要人员和门店匹配
    # 因为同一套房源可能由不同的人或门店处理
...
def process_property_image(image_path):
    ...
    # 中盘喜报过滤规则检查，但暂时不过滤，先用于面积互补
    # 保存过滤状态到 _skip_write 字段
    should_skip = False
    if property_info["类型"] == "中盘":
        ...
        if property_info["门店"] not in allowed_stores:
            print(f"中盘记录标记为待跳过：维护人门店 {property_info['门店']} 不在允许列表中")
            should_skip = True
        ...
        if not should_skip:
            ...
            if not unique_stores:
                ...
                if not has_other_store:
                    print(f"中盘记录标记为待跳过：维护人门店 {property_info['门店']} 与成交人门店相同")
                    should_skip = True
                else:
                ...
                if any(store in allowed_stores for store in unique_stores):
                    print(f"中盘记录标记为待跳过：维护人门店 {property_info['门店']} 和成交人门店 {unique_stores} 都在允许列表中")
                    should_skip = True
                else:
...
def _send_wps_request(action, payload_data, headers=None):
    ...
    # 调试：保存发送的请求到文件
    try:
        debug_file = os.path.join(config.output_dir, "last_wps_request.json")
        with open(debug_file, "w", encoding="utf-8") as f:
            import json
            json.dump(payload, f, ensure_ascii=False, indent=2)
        print(f"  [调试] 已保存 WPS 请求到: {debug_file}")
    except Exception as e:
        print(f"  [调试] 保存 WPS 请求失败: {str(e)}")
...
def write_data_to_wps_sheet(data_list, days=14):
    ...
    # 调试信息：检查传入的记录类型
    print("\n=== write_data_to_wps_sheet 接收记录检查：")
    print(f"  接收记录数: {len(data_list)}")
    for i, r in enumerate(data_list):
        print(f"    记录 {i+1}: 类型={r.get('类型')}, 小区={r.get('小区')}, 人员={r.get('人员')}")
    print(f"  其中中盘记录数: {sum(1 for r in data_list if r.get('类型') == '中盘')}")
    ...
    # 2. 转换现有记录为字典，便于查重和更新
    # 使用全局变量 _full_records_with_id，它包含完整的记录信息（包括 id）
    full_existing_dict = {}  # 用于完整查重（包含金额）
    for record in _full_records_with_id:
        if isinstance(record, dict):
            fields = record.get("fields", {})
            if isinstance(fields, dict):
                # 构建唯一标识：日期+类型+小区+门店+人员（不包含金额和面积，用于检测需要更新的记录）
                update_key = f"{fields.get('日期', '').replace('/', '-')}_{fields.get('类型', '')}_{fields.get('小区', '')}_{fields.get('门店', '')}_{fields.get('人员', '')}"
                # 构建完整查重键（包含金额）
                full_key = f"{fields.get('日期', '').replace('/', '-')}_{fields.get('类型', '')}_{fields.get('小区', '')}_{fields.get('金额', '')}_{fields.get('门店', '')}_{fields.get('人员', '')}"
                
                existing_dict[update_key] = True  # 用于更新检测
                full_existing_dict[full_key] = True  # 用于完整查重
                # 保存完整记录信息，包括ID
                existing_records_full[update_key] = {
                    'id': record.get('id', ''),
                    'fields': fields
                }
    print(f"构建了包含 {len(existing_dict)} 条记录的查重和更新字典")
    ...
    def build_wps_record(result):
        ...
        返回:
            tuple - (wps_record, key, update_key) 转换后的WPS记录和查重键
        """
        ...
        # 构建更新检测键（不包含金额和面积）
        update_key = f"{result['日期']}_{record_type}_{result['小区']}_{result['门店']}_{result['人员']}"
        
        return wps_record, key, update_key
    ...
    for i, result in enumerate(data_list):
        ...
        if result.get('_skip_write', False):
            skip_count += 1
            area = result.get("面积", "")
            print(f"  ⚠️  记录 {i+1}：{result['类型']} - {result['人员']} - {result['小区']} - {area} - {result['金额']} (全部不在已知列表，跳过写入)")
            continue
        
        # 使用内部辅助函数构建记录和查重键
        wps_record, key, update_key = build_wps_record(result)
        
        # 检查是否为重复记录或需要更新的记录
        area = result.get("面积", "")
        amount = result.get("金额", "")
        
        if update_key in existing_records_full:
            # 找到匹配的现有记录，检查是否需要更新
            existing_record = existing_records_full[update_key]
            existing_fields = existing_record['fields']
            existing_area = existing_fields.get('面积', '')
            existing_amount = existing_fields.get('金额', '')
            
            # 检查是否有需要更新的字段：面积从空到有，金额从0到有
            needs_update = False
            fields_to_update = {}
            update_reasons = []
            
            # 检查面积
            if (not existing_area or str(existing_area).strip() == '') and area and str(area).strip() != '':
                fields_to_update['面积'] = area
                update_reasons.append(f"面积从空更新为{area}")
                needs_update = True
            
            # 检查金额
            if (str(existing_amount) == '0' or not existing_amount) and amount and str(amount) != '0':
                fields_to_update['金额'] = amount
                update_reasons.append(f"金额从{existing_amount}更新为{amount}")
                needs_update = True
            ...
            if needs_update:
                ...
                update_count += 1
                print(f"  🔄 记录 {i+1}：{result['类型']} - {result['人员']} - {result['小区']} - {area} - {result['金额']} (检测到可更新字段: {', '.join(update_reasons)})")
            else:
                # 完全重复，无需更新
                duplicate_count += 1
                print(f"  ❌ 记录 {i+1}：{result['类型']} - {result['人员']} - {result['小区']} - {area} - {result['金额']} (重复记录，跳过)")
        elif key in full_existing_dict:
            # 完全重复，无需更新
            duplicate_count += 1
            print(f"  ❌ 记录 {i+1}：{result['类型']} - {result['人员']} - {result['小区']} - {area} - {result['金额']} (重复记录，跳过)")
        else:
            ...
            full_existing_dict[key] = True
            print(f"  ✅ 记录 {i+1}：{result['类型']} - {result['人员']} - {result['小区']} - {area} - {result['金额']} (准备写入)")
    ...
    # 调试信息：检查最终要写入的记录
    print(f"\n=== 最终要处理的记录检查：")
    print(f"  要写入的新记录: {len(wps_records)} 条")
    for i, r in enumerate(wps_records):
        print(f"    新记录 {i+1}: 类型={r.get('类型')}, 小区={r.get('小区')}, 人员={r.get('人员')}")
    print(f"  要更新的记录: {len(records_to_update)} 条")
    print(f"  其中中盘记录数: {sum(1 for r in wps_records if r.get('类型') == '中盘')}")
    
    print(f"\n写入统计：")
    print(f"待写入记录：{len(data_list)}, 重复记录：{duplicate_count}, 全部不在已知列表：{skip_count}, 待更新记录：{update_count}, 准备写入：{len(wps_records)}")
    
    # 如果没有记录需要写入或更新，直接返回成功
    if not wps_records and not records_to_update:
        print("没有记录需要写入或更新")
        _save_write_count(0)
        return True
    ...
    if not _is_wps_enabled():
        print(f"\n调试模式：跳过实际写入和更新")
        print(f"  待写入记录：{len(wps_records)} 条")
        print(f"  待更新记录：{update_count} 条")
    ...
    if records_to_update:
        print(f"\n=== 开始更新记录 ===")
        print(f"待更新记录数：{len(records_to_update)}")
        for record_update in records_to_update:
            record_id = record_update['record_id']
            fields = record_update['fields']
            index = record_update['index']
            result = record_update['result']
            
            print(f"  更新记录 {index} (ID: {record_id})：{result['类型']} - {result['人员']} - {result['小区']}")
            print(f"    更新字段：{fields}")
            
            if update_record_in_wps(record_id, fields):
                actual_update_count += 1
            time.sleep(0.1)  # 避免请求过于频繁
        
        print(f"更新完成：成功更新 {actual_update_count}/{len(records_to_update)} 条记录")
    ...
    if wps_records:
        print(f"\n=== 开始写入新记录 ===")
        # 打印发送给 WPS 的请求数据，调试用
        print(f"  准备发送 {len(wps_records)} 条记录到 WPS：")
        for i, rec in enumerate(wps_records):
            print(f"    新记录 {i+1}: {rec}")
        # 使用通用请求函数发送写入请求
        ...
        if actual_result.get("success", False):
            # 打印完整的 WPS 返回结果，调试用
            print(f"  [调试] WPS 返回的完整结果：")
            import json
            print(json.dumps(actual_result, ensure_ascii=False, indent=2))
            ...
            print(f"成功写入 {actual_write_count} 条记录到 WPS 多维表格")
    ...
    # 保存实际处理的总记录数（写入+更新）到临时文件
    total_processed = actual_write_count + actual_update_count
    _save_write_count(total_processed)
    
    print(f"\n=== 完成总结 ===")
    print(f"写入新记录：{actual_write_count}/{len(wps_records)} 条")
    print(f"更新现有记录：{actual_update_count}/{len(records_to_update)} 条")
...
def update_record_in_wps(record_id, fields):
    """
    更新WPS多维表格中的单个记录
    ...
    try:
        # 使用通用请求函数发送更新请求
        actual_result = _send_wps_request("update", {"recordId": record_id, "fields": fields})
        ...
        if success is True or success == "true":
            ...
            print(f"更新记录失败 {record_id}：{message}")
            return False
...
def find_matching_wps_record_pairs(records):
    """
    查找匹配的二手和中盘记录对
    
    参数:
        records: list - 所有记录列表，每个记录包含id和fields
    ...
    for record in records:
        ...
        area = fields.get('小区', '')
        if area:
            if area not in records_by_area:
                records_by_area[area] = []
            records_by_area[area].append(record)
    
    # 匹配
    for area, records_in_area in records_by_area.items():
        if len(records_in_area) < 2:
            continue
            
        secondhand_records = [r for r in records_in_area if r.get('fields', {}).get('类型') == '二手']
        medium_records = [r for r in records_in_area if r.get('fields', {}).get('类型') == '中盘']
        
        # 两两匹配
        for sh_rec in secondhand_records:
            sh_fields = sh_rec.get('fields', {})
            sh_area = sh_fields.get('面积', '')
            sh_amount = sh_fields.get('金额', 0)
            needs_area = not sh_area or str(sh_area).strip() == ''
            
            if not needs_area:
                continue
            
            for m_rec in medium_records:
                m_fields = m_rec.get('fields', {})
                m_amount = m_fields.get('金额', 0)
                needs_amount = str(m_amount) == '0' or not m_amount
                
                if not needs_amount:
                    continue
                
                # 匹配条件：日期相同
                if sh_fields.get('日期') == m_fields.get('日期'):
                    record_pairs.append({
                        'secondhand': sh_rec,
                        'medium': m_rec,
...
def supplement_wps_data(days=90):
    """
    从WPS读取数据，自动补充缺失字段
    ...
    # 获取WPS中最近N天的记录
    print(f"\n正在获取WPS表格中最近{days}天的记录...")
    all_records = get_existing_wps_records(sort_records=False, days=days)
    ...
    # 查找需要补充的记录对
    print(f"\n正在查找需要补充的记录对...")
    match_pairs = find_matching_wps_record_pairs(_full_records_with_id)
    
    if not match_pairs:
        print("没有找到需要补充的记录对")
        return True
    
    print(f"找到 {len(match_pairs)} 对记录需要补充")
    
    # 显示记录对
    print(f"\n=== 需要补充的记录对：")
    for i, pair in enumerate(match_pairs, 1):
        sh_rec = pair['secondhand']
        m_rec = pair['medium']
        sh_fields = sh_rec.get('fields', {})
        m_fields = m_rec.get('fields', {})
        
        print(f"\n{i}. 二手记录：")
        print(f"   ID: {sh_rec.get('id', '')}")
        print(f"   日期: {sh_fields.get('日期', '')}")
        print(f"   小区: {sh_fields.get('小区', '')}")
        print(f"   面积: {sh_fields.get('面积', '')}")
        print(f"   金额: {sh_fields.get('金额', '')}")
        print(f"   中盘记录：")
        print(f"   ID: {m_rec.get('id', '')}")
        print(f"   面积: {m_fields.get('面积', '')}")
        print(f"   金额: {m_fields.get('金额', '')}")
    
    # 询问是否继续
    try:
        choice = input(f"\n确认补充上述记录吗？(y/n): ").strip().lower()
        if choice != 'y':
            print("取消操作")
            return False
    ...
    for i, pair in enumerate(match_pairs, 1):
        ...
        # 补充面积
        if pair['needs_area']:
            m_area = m_fields.get('面积', '')
            if m_area:
                print(f"更新二手记录 {sh_rec.get('id', '')} 的面积: {m_area}")
                if update_record_in_wps(sh_rec.get('id', ''), {'面积': m_area}):
                    success_count += 1
                    time.sleep(0.5)
        
        # 补充金额
        if pair['needs_amount']:
            sh_amount = sh_fields.get('金额', 0)
            if sh_amount and str(sh_amount) != '0':
                print(f"更新中盘记录 {m_rec.get('id', '')} 的金额: {sh_amount}")
                if update_record_in_wps(m_rec.get('id', ''), {'金额': str(sh_amount)}):
                    success_count += 1
                    time.sleep(0.5)
    
    print(f"\n数据补充完成，共更新 {success_count} 条记录")
    return True
...
def print_results_summary(all_results):
    ...
    print("\n房产喜报识别结果汇总（按精确时间排序）")
    print("=" * 120)
    print(f"{headers[0]:<10} {headers[1]:<8} {headers[2]:<15} {headers[3]:<8} {headers[4]:<10} {headers[5]:<6} {headers[6]:<8}")
    print("=" * 120)
    ...
    # 先打印正常记录
    for result in normal_records:
        # 内联format_result_line函数的逻辑
        note = result.get("备注", "")
        area = result.get("面积", "")
        main_data = f"{result['日期']:<10} {result['类型']:<8} {result['小区']:<15} {area:<8} {result['金额']:<10} {result['门店']:<6} {result['人员']:<8}"
        lines = [main_data]
        if note:
            lines.append(f"    {note}")
    ...
    if skipped_records:
        print("\n" + "=" * 120)
        print(f"待跳过记录（不会写入WPS，共 {len(skipped_records)} 条）：")
        print("=" * 120)
        for result in skipped_records:
            # 内联format_result_line函数的逻辑
            note = result.get("备注", "")
            area = result.get("面积", "")
            main_data = f"{result['日期']:<10} {result['类型']:<8} {result['小区']:<15} {area:<8} {result['金额']:<10} {result['门店']:<6} {result['人员']:<8}"
    ...
    print("=" * 120)
    print(f"共 {len(all_results)} 条记录（正常：{len(normal_records)} 条，待跳过：{len(skipped_records)} 条）")
    ...
    try:
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write("=" * 120 + "\n")
            f.write(f"{headers[0]:<10} {headers[1]:<8} {headers[2]:<15} {headers[3]:<8} {headers[4]:<10} {headers[5]:<6} {headers[6]:<8}\n")
            f.write("=" * 120 + "\n")
            for line in output_lines:
                f.write(line + "\n")
            f.write("=" * 120 + "\n")
            f.write(f"共 {len(all_results)} 条记录（正常：{len(normal_records)} 条，待跳过：{len(skipped_records)} 条）\n")
...
def main():
    ...
    # 先进行二手和中盘喜报关联匹配与信息互补（在汇总之前）
    if all_results:
        all_results = match_and_complement_secondhand_medium(all_results)
    ...
    # 写入数据到WPS多维表格或缓存文件
    if all_results:
        # 准备写入WPS的数据，跳过被标记为不写入的记录（在面积互补之后构建）
        wps_data = []
        print("\n=== 构建待写入记录检查：")
        for i, result in enumerate(all_results):
            record_type = result.get("类型", "未知")
            print(f"  记录 {i+1}: 类型={record_type}, 小区={result.get('小区')}, 人员={result.get('人员')}")
            # 检查是否标记为跳过写入
            if result.get('_skip_write', False):
                continue
        ...
        print(f"构建完成，待写入记录数: {len(wps_data)}")
        print(f"其中中盘记录数: {sum(1 for r in wps_data if r.get('类型') == '中盘')}")
        print(f"  wps_data 完整内容：")
        for i, r in enumerate(wps_data):
            print(f"    {i+1}: {r}")
...
def write_cache_data(days=30, selected_indices=None, force_write=False):
    ...
    print("\n=== 从缓存文件读取记录检查：")
    if wps_data_cached:
        print(f"  从缓存读取到 {len(wps_data_cached)} 条待写入记录")
        for i, r in enumerate(wps_data_cached):
            print(f"    记录 {i+1}: 类型={r.get('类型')}, 小区={r.get('小区')}, 人员={r.get('人员')}")
        print(f"  其中中盘记录数: {sum(1 for r in wps_data_cached if r.get('类型') == '中盘')}")
    ...
    if force_write and selected_indices is not None:
        ...
        # 调试信息：检查强制写入的记录类型
        print("\n=== 强制写入记录检查：")
        for i, r in enumerate(wps_data_to_write):
            print(f"    记录 {i+1}: 类型={r.get('类型')}, 小区={r.get('小区')}, 人员={r.get('人员')}")
        print(f"  其中中盘记录数: {sum(1 for r in wps_data_to_write if r.get('类型') == '中盘')}")
